...
export class Keyer {
  ...
  oscillatev2() {
    if (this.mode == 2 && !this.ditKeyState && !this.dahKeyState) {
      // Iambic B doesn't clear the queue
      if (this.streak > 1) {
  ...
  oscillatev3() {
    if (
      ...
      // Iambic B doesn't clear the queue
      if (
        (this.ditStreak && this.queue[0] == 1) ||
        (this.dahStreak && this.queue[0] == 2)
      ) {
        console.log(
          'ditStreak: ' +
            this.ditStreak +
    ...
    if (this.ditKeyState) {
      ...
      } else if (!this.dahKeyState && this.queue[0] == 2) {
        // dah was canceled.  Replace in queue
        this.queue[0] = 1;
        this.dahStreak = 0;
        this.ditStreak++;
      }
    ...
    if (this.dahKeyState) {
      ...
      } else if (!this.ditKeyState && this.queue[0] == 1) {
        // dit was canceled.  Replace in queue
        this.queue[0] = 2;
        this.ditStreak = 0;
        this.dahStreak++;
      }
  ...
  oscillate() {
    if (
      ...
      if (this.queue[0] == 1) {
        // Dit is in the queue
        if (
          this.ditStart < this.dahStart ||
          this.ditStop - this.ditStart > this.unit * 4
        ) {
          this.queue.pop();
        }
      } else {
        // Dah is in the queue
        if (
          this.dahStart < this.ditStart ||
          this.dahStop - this.dahStart > this.unit * 2
        ) {
          this.queue.pop();
        }
      }
