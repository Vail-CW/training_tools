...
The project now includes five training modules:
1. **Copy Practice** - Basic Morse code receiving practice (Hear It, Type It)
2. **Hear It, Say It** - Speech recognition training for instant character recognition (ICR)
3. **Send Practice** - Morse code sending practice with USB CW key support
4. **Free Practice** - Freeform morse sending with real-time decoding and MP3 recording
5. **QSO Simulator** - Advanced pileup training for contesting and field operations
...
### Hear It, Say It Module
- **TypeScript** with Vite build system
- **Web Speech API** - Browser speech recognition (Google Speech API backend)
- **Web Audio API** - For precise Morse code tone generation
- **PWA** - Offline-capable with service worker
- **Zero runtime dependencies** - Pure vanilla TypeScript
...
- **Hybrid approach:** Main training tools are pure static; Hear It Say It uses Vite; QSO Simulator uses Webpack
...
│   ├── hear-it-say-it/          # Hear It, Say It module (speech recognition ICR)
│   │   ├── src/                 # TypeScript source files
│   │   │   ├── components/      # UI components (settings, chart, mode selector)
│   │   │   ├── core/            # Core logic (speech, audio, adaptive selector)
│   │   │   ├── data/            # Static data (morse codes, word lists)
│   │   │   ├── services/        # Business logic (practice session)
...
### Hear It, Say It (Implemented)
A speech recognition training module for practicing Instant Character Recognition (ICR) by speaking letters, numbers, and callsigns aloud. Uses the Web Speech API to transcribe spoken input and compares it against morse code patterns.

**Practice Modes:**
1. **Character Mode** - Single letter recognition (A-Z, 0-9)
2. **Word Mode** - Multi-character word recognition with length filtering (3-5 chars)
3. **Callsign Mode** - Amateur radio callsign patterns with format selection (BETA)

**Key Features:**
- **Web Speech API** - Browser speech recognition with NATO phonetic support
- **Adaptive Learning** - Weighted selection that favors weak characters
- **Performance Tracking** - Response time measurement, ICR metrics, history visualization
- **Voice Calibration** - Personal voice profile training for improved accuracy
- **PWA Support** - Installable as native app with offline capability

**Settings:**
- Speed: 15-40 WPM (default 25 WPM)
- Adaptive Gain: 0-100 (controls weighting of weak characters)
- Tone Frequency: 400-1000 Hz (default 700 Hz)
- Repeat until correct toggle
- Strict NATO mode toggle
- Character set selection with custom overrides
- History import/export (JSON format)

**Platform Support:**
- **Desktop:** Chrome, Edge, Safari
- **iOS:** Safari, Chrome
- **Android:** Not supported (speech recognition limitations)

**Build Commands:**
```bash
cd training/hear-it-say-it
...
**File Locations:**
- Entry point: [training/hear-it-say-it/index.html](training/hear-it-say-it/index.html)
- Main app logic: [training/hear-it-say-it/src/main.ts](training/hear-it-say-it/src/main.ts)
- Practice session: [training/hear-it-say-it/src/services/practice-session.ts](training/hear-it-say-it/src/services/practice-session.ts)
- Speech recognition: [training/hear-it-say-it/src/core/speech-recognition.ts](training/hear-it-say-it/src/core/speech-recognition.ts)
- Morse audio: [training/hear-it-say-it/src/core/morse-audio.ts](training/hear-it-say-it/src/core/morse-audio.ts)
- State management: [training/hear-it-say-it/src/state/store.ts](training/hear-it-say-it/src/state/store.ts)
- Styles: [training/hear-it-say-it/src/styles/main.css](training/hear-it-say-it/src/styles/main.css)
...
### December 4, 2025 - Hear It, Say It Module Integration
- **Integrated Hear It, Say It module** from separate "speak it test" project
- **Speech recognition training** for Instant Character Recognition (ICR)
- **Three practice modes:** Character, Word, and Callsign (beta)
- **Technology stack:** TypeScript + Vite build system, Web Speech API
- **Features:**
  - Adaptive learning with weighted character selection
  - Response time tracking and performance visualization
  - NATO phonetic alphabet support (flexible and strict modes)
  - Voice calibration for personal speaker adaptation
  - PWA support with offline capability
- **Platform support:** Desktop (Chrome, Edge, Safari), iOS; Android not supported
- **File locations:** `training/hear-it-say-it/` with src/, dist/, and config files
- **Build commands:** `npm run dev` for development, `npm run build` for production
- Updated landing page card from "Coming Soon" to active link with BETA badge
...
*Project Status: BETA - All Modules Functional with USB CW Key Support, MP3 Recording, and Speech Recognition*
